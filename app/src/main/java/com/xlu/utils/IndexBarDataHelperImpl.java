        ...
        for (int i = 0; i < size; i++) {
            ...
            if (indexPinyinBean.isNeedToPinyin()) {
                ...
                for (int i1 = 0; i1 < target.length(); i1++) {
                    ...
                    Pinyin.toPinyin(target.charAt(i1));
                    pySb.append(Pinyin.toPinyin(target.charAt(i1)).toUpperCase());
                }
                if(target.equals("重庆市")){
                    indexPinyinBean.setBaseIndexPinyin("CHONGQING");//设置城市名全拼音
                }else if(target.equals("长沙市")){
                    indexPinyinBean.setBaseIndexPinyin("CHANGSHASHI");//设置城市名全拼音
                }else
